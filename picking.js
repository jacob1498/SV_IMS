let pickingLogs = [];
let userMasterData = [];
let db;
let pickCurrentPage = 1;
const pickRowsPerPage = 5;
let currentPickTab = 'Picking';
let tempStartData = null;
let summaryCurrentPage = 1;
const summaryRowsPerPage = 5;
let currentSummaryType = '';
let currentSort = { field: 'id', direction: 'desc' };

// Lock Screen Logic
let idleTimer;
const IDLE_TIME = 15 * 60 * 1000; // 15 Minutes
const LOCK_CODE = "SV123";

function resetIdleTimer() {
    clearTimeout(idleTimer);
    const lockModal = document.getElementById('lockScreenModal');
    if (!lockModal || lockModal.style.display === 'none' || lockModal.style.display === '') {
        idleTimer = setTimeout(lockScreen, IDLE_TIME);
    }
}

function lockScreen() {
    const lockModal = document.getElementById('lockScreenModal');
    if (lockModal) {
        lockModal.style.display = 'flex';
        document.getElementById('lockCodeInput').value = '';
        document.getElementById('lockCodeInput').focus();
        localStorage.setItem('isLocked', 'true');
    }
}

function unlockScreen() {
    const code = document.getElementById('lockCodeInput').value;
    if (code === LOCK_CODE) {
        document.getElementById('lockScreenModal').style.display = 'none';
        showToast("System unlocked", "success");
        sessionStorage.setItem('authenticated', 'true');
        localStorage.removeItem('isLocked');
        resetIdleTimer();
    } else {
        showToast("Incorrect security code", "danger");
        document.getElementById('lockCodeInput').value = '';
        const lockContent = document.querySelector('.lock-content');
        if (lockContent) {
            lockContent.classList.add('shake');
            setTimeout(() => lockContent.classList.remove('shake'), 400);
        }
    }
}

const dbRequest = indexedDB.open("SunnyvilleTrackerDB", 3);
dbRequest.onsuccess = (e) => {
    db = e.target.result;
    loadData();
};

// Update live durations every 30 seconds
setInterval(() => {
    if (currentPickTab === 'Picking') renderPickingTable();
    if (document.getElementById('summaryModal').style.display === 'block') renderSummaryModalContent();
}, 30000);

// Global events to track activity for auto-lock
window.onload = () => {
    resetIdleTimer();
    showTableSkeletons('pickingBody', 5, 9);
};

if (sessionStorage.getItem('authenticated') !== 'true' || localStorage.getItem('isLocked') === 'true') {
    lockScreen();
}
window.onmousemove = resetIdleTimer;

function showTableSkeletons(tbodyId, rows, cols) {
    const tbody = document.getElementById(tbodyId);
    if (!tbody) return;
    let html = '';
    for (let i = 0; i < rows; i++) {
        html += `<tr>${'<td><div class="skeleton"></div></td>'.repeat(cols)}</tr>`;
    }
    tbody.innerHTML = html;
}

window.onmousedown = resetIdleTimer;
window.ontouchstart = resetIdleTimer;
window.onkeypress = resetIdleTimer;

dbRequest.onupgradeneeded = function(event) {
    const database = event.target.result;
    if (!database.objectStoreNames.contains("attendanceLogs")) {
        database.createObjectStore("attendanceLogs", { keyPath: ["userId", "date"] });
    }
    if (!database.objectStoreNames.contains("userMasterData")) {
        database.createObjectStore("userMasterData", { keyPath: "id" });
    }
    if (!database.objectStoreNames.contains("pickingLogs")) {
        database.createObjectStore("pickingLogs", { keyPath: "id", autoIncrement: true });
    }
    if (!database.objectStoreNames.contains("cycleCountLogs")) {
        database.createObjectStore("cycleCountLogs", { keyPath: "id", autoIncrement: true });
    }
};

function loadData() {
    const tx = db.transaction(["pickingLogs", "userMasterData"], "readonly");
    tx.objectStore("pickingLogs").getAll().onsuccess = (e) => {
        pickingLogs = e.target.result;
        renderPickingTable();
    };
    tx.objectStore("userMasterData").getAll().onsuccess = (e) => {
        userMasterData = e.target.result;
    };
}

// Goal 1: Immediate name lookup during scanning/typing
document.getElementById('pickingUserId').addEventListener('input', (e) => {
    const input = e.target.value.trim();
    if (input.includes(',')) {
        const ids = input.split(',').map(id => id.trim()).filter(id => id !== '');
        const names = ids.map(id => {
            const user = userMasterData.find(u => u.id === id);
            return user ? user.name : null;
        }).filter(name => name !== null);
        
        document.getElementById('pickingUserName').value = names.length > 0 ? names.join('\n') : '';
    } else {
        const user = userMasterData.find(u => u.id === input);
        document.getElementById('pickingUserName').value = user ? user.name : '';
    }
});

function showToast(message, type = 'info') {
    const container = document.getElementById('toastContainer');
    const toast = document.createElement('div');
    toast.className = `toast ${type}`;
    
    const icon = type === 'success' ? 'check-circle' : (type === 'danger' ? 'exclamation-circle' : 'info-circle');
    toast.innerHTML = `<i class="fas fa-${icon}"></i> <span>${message}</span>`;
    
    container.appendChild(toast);
    setTimeout(() => {
        toast.style.opacity = '0';
        setTimeout(() => toast.remove(), 300);
    }, 4000);
}

function processPicking(action) {
    const userIdInput = document.getElementById('pickingUserId').value.trim();
    const batchId = document.getElementById('batchId').value.trim();
    
    const now = new Date();
    const timeStr = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const dateStr = now.toISOString().split('T')[0];

    if (action === 'START') {
        const userIds = userIdInput.split(',').map(id => id.trim()).filter(id => id !== '');
        if (userIds.length === 0) return showToast("Enter at least one User ID", "warning");
        if (!batchId) return showToast("Batch ID is required to start!", "warning");

        // Goal 2: Ensure the IDs are registered
        const unregistered = userIds.filter(id => !userMasterData.some(u => u.id === id));
        if (unregistered.length > 0) return showToast(`Unregistered IDs: ${unregistered.join(', ')}`, "danger");

        // Prevent starting if user is already picking
        const busy = userIds.filter(id => pickingLogs.some(l => l.userId === id && l.endTime === '-'));
        if (busy.length > 0) return showToast(`Users already picking: ${busy.join(', ')}`, "warning");

        tempStartData = {
            ids: userIds,
            batchId,
            date: dateStr,
            startTime: timeStr,
            endTime: '-',
            duration: '-'
        };
        document.getElementById('zoneModal').style.display = 'block';
    } else {
        const log = pickingLogs.slice().reverse().find(l => l.userId === userIdInput && l.endTime === '-');
        if (!log) return showToast("No active picking found for this user", "warning");

        log.endTime = timeStr;
        log.duration = calculateDuration(log.startTime, log.endTime);

        db.transaction("pickingLogs", "readwrite").objectStore("pickingLogs").put(log).onsuccess = () => {
            completeAction("END", userIdInput);
        };
    }
}

function confirmStartPicking(zone) {
    if (!tempStartData) return;
    
    const tx = db.transaction("pickingLogs", "readwrite");
    const store = tx.objectStore("pickingLogs");

    tempStartData.ids.forEach(id => {
        const user = userMasterData.find(u => u.id === id);
        const newLog = {
            userId: id,
            name: user.name,
            company: user.company || '-',
            batchId: tempStartData.batchId,
            date: tempStartData.date,
            startTime: tempStartData.startTime,
            endTime: tempStartData.endTime,
            duration: tempStartData.duration,
            zone: zone
        };
        store.add(newLog).onsuccess = (e) => {
            newLog.id = e.target.result;
            pickingLogs.push(newLog);
        };
    });

    tx.oncomplete = () => {
        completeAction("START", tempStartData.ids.join(', '));
        document.getElementById('zoneModal').style.display = 'none';
        tempStartData = null;
    };
}

function calculateDuration(start, end) {
    const parse = (s) => {
        let [t, m] = s.split(' ');
        let [h, min] = t.split(':').map(Number);
        if (m === 'PM' && h !== 12) h += 12;
        if (m === 'AM' && h === 12) h = 0;
        return h * 60 + min;
    };
    let diff = parse(end) - parse(start);
    // Handle crossing midnight
    if (diff < 0) diff += 1440; 
    return diff > 0 ? `${Math.floor(diff/60)}h ${diff%60}m` : `${diff}m`;
}

function getDurationMinutes(start, end) {
    const parse = (s) => {
        let [t, m] = s.split(' ');
        let [h, min] = t.split(':').map(Number);
        if (m === 'PM' && h !== 12) h += 12;
        if (m === 'AM' && h === 12) h = 0;
        return h * 60 + min;
    };
    let diff = parse(end) - parse(start);
    return diff < 0 ? diff + 1440 : diff;
}

function completeAction(type, id) {
    showToast(`Picking ${type} recorded for ${id}`, "success");
    document.getElementById('pickingUserId').value = '';
    document.getElementById('batchId').value = '';
    document.getElementById('pickingUserName').value = '';
    pickCurrentPage = 1;
    renderPickingTable();
    
    const status = document.getElementById('scanStatus');
    status.innerText = `LAST ACTION: ${type} (${id})`;
    setTimeout(() => status.innerText = 'READY TO SCAN', 3000);
}

function sortData(field) {
    if (currentSort.field === field) {
        currentSort.direction = currentSort.direction === 'asc' ? 'desc' : 'asc';
    } else {
        currentSort.field = field;
        currentSort.direction = 'asc';
    }
    renderPickingTable();
}

function renderPickingTable() {
    const tbody = document.getElementById('pickingBody');
    const search = document.getElementById('pickSearch').value.toLowerCase();
    const zoneSearch = document.getElementById('zoneSearch').value;
    const dateFrom = document.getElementById('dateFrom').value;
    const dateTo = document.getElementById('dateTo').value;
    tbody.innerHTML = '';

    let filtered = pickingLogs.filter(l => 
        (l.userId.toLowerCase().includes(search) || 
         l.batchId.toLowerCase().includes(search) ||
         (l.name && l.name.toLowerCase().includes(search))) &&
        (zoneSearch === "" || l.zone === zoneSearch) &&
        (!dateFrom || l.date >= dateFrom) &&
        (!dateTo || l.date <= dateTo) &&
        (currentPickTab === 'Picking' ? l.endTime === '-' : l.endTime !== '-')
    );

    // Apply Sorting
    filtered.sort((a, b) => {
        let valA = a[currentSort.field] || '';
        let valB = b[currentSort.field] || '';
        if (typeof valA === 'string') { valA = valA.toLowerCase(); valB = valB.toLowerCase(); }
        
        if (valA < valB) return currentSort.direction === 'asc' ? -1 : 1;
        if (valA > valB) return currentSort.direction === 'asc' ? 1 : -1;
        return 0;
    });

    // Goal 3: Pagination logic
    const totalPages = Math.ceil(filtered.length / pickRowsPerPage) || 1;
    if (pickCurrentPage > totalPages) pickCurrentPage = totalPages;
    const start = (pickCurrentPage - 1) * pickRowsPerPage;
    const paginatedData = filtered.slice(start, start + pickRowsPerPage);

    const nowTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    let tableHtml = '';

    paginatedData.forEach(log => {
        let displayDuration = log.duration;
        // Live duration for active picking
        if (log.endTime === '-') {
            displayDuration = `<span style="color: var(--accent); font-weight: bold;"><i class="fas fa-sync fa-spin"></i> ${calculateDuration(log.startTime, nowTime)}</span>`;
        }

        tableHtml += `
            <tr>
                <td><strong>${log.userId}</strong><br><small>${log.name || 'Unknown'}</small></td>
                <td>${log.company || '-'}</td>
                <td>${log.batchId}</td>
                <td>${log.zone || '-'}</td>
                <td>${log.date}</td>
                <td>${log.startTime}</td>
                <td>${log.endTime}</td>
                <td>${displayDuration}</td>
                <td>
                    ${log.endTime === '-' ? `<button class="edit-btn" onclick="quickEndPick(${log.id})" title="End Picking"><i class="fas fa-stop"></i></button>` : ''}
                    <button class="delete-btn" onclick="deletePick(${log.id})"><i class="fas fa-trash"></i></button>
                </td>
            </tr>
        `;
    });
    tbody.innerHTML = tableHtml;
    renderPickPagination(filtered.length);
    updatePickStats();
}

function updatePickStats() {
    const activePicks = pickingLogs.filter(l => l.endTime === '-');
    const today = new Date().toISOString().split('T')[0];
    const finishedToday = pickingLogs.filter(l => l.endTime !== '-' && l.date === today);
    const nowTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

    // Unique Personnel Count
    const uniqueUserIds = [...new Set(activePicks.map(l => l.userId))];
    document.getElementById('stat-uniquePickers').innerText = uniqueUserIds.length;

    // Unique Batches
    const uniqueBatchesPicking = [...new Set(activePicks.map(l => l.batchId))].length;
    const uniqueBatchesPicked = [...new Set(finishedToday.map(l => l.batchId))].length;

    document.getElementById('stat-uniqueBatchesPicking').innerText = uniqueBatchesPicking;
    document.getElementById('stat-uniqueBatchesPicked').innerText = uniqueBatchesPicked;

    // Long Picks (> 1 Hour)
    const longPicks = activePicks.filter(l => getDurationMinutes(l.startTime, nowTime) > 60);
    const longPicksCount = longPicks.length;
    document.getElementById('stat-longPicks').innerText = longPicksCount;

    const longPickCard = document.getElementById('longPickCard');
    if (longPicksCount > 0) {
        longPickCard.classList.add('blink-danger');
    } else {
        longPickCard.classList.remove('blink-danger');
    }
}

function openSummaryModal(type) {
    currentSummaryType = type;
    summaryCurrentPage = 1;
    renderSummaryModalContent();
    document.getElementById('summaryModal').style.display = 'block';
}

function renderSummaryModalContent() {
    const tbody = document.getElementById('summaryModalBody');
    const title = document.getElementById('summaryModalTitle');
    const today = new Date().toISOString().split('T')[0];
    const nowTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

    let list = [];
    if (currentSummaryType === 'active') {
        title.innerText = "Active Pickers";
        list = pickingLogs.filter(l => l.endTime === '-');
    } else if (currentSummaryType === 'longPick') {
        title.innerText = "Long Picking Sessions (>1hr)";
        list = pickingLogs.filter(l => l.endTime === '-' && getDurationMinutes(l.startTime, nowTime) > 60);
    } else if (currentSummaryType === 'batchesPicking') {
        title.innerText = "Active Batches";
        list = pickingLogs.filter(l => l.endTime === '-');
    } else if (currentSummaryType === 'batchesPicked') {
        title.innerText = "Batches Picked Today";
        list = pickingLogs.filter(l => l.endTime !== '-' && l.date === today);
    }

    const totalPages = Math.ceil(list.length / summaryRowsPerPage) || 1;
    if (summaryCurrentPage > totalPages) summaryCurrentPage = totalPages;
    const start = (summaryCurrentPage - 1) * summaryRowsPerPage;
    const paginated = list.slice(start, start + summaryRowsPerPage);

    let summaryHtml = '';
    paginated.forEach(log => {
        summaryHtml += `
            <tr>
                <td><strong>${log.userId}</strong><br><small>${log.name}</small></td>
                <td>${log.batchId}</td>
                <td>
                    ${log.endTime === '-' ? `<button class="edit-btn" onclick="quickEndPick(${log.id})">End Pick</button>` : '<span class="status-badge" style="background:#ebfaeb; color:#27ae60">Done</span>'}
                </td>
            </tr>
        `;
    });
    tbody.innerHTML = summaryHtml;
    renderSummaryPagination(list.length);
}

function renderSummaryPagination(totalItems) {
    const totalPages = Math.ceil(totalItems / summaryRowsPerPage) || 1;
    const container = document.getElementById('summaryPaginationNumbers');
    if (!container) return;
    container.innerHTML = '';

    for (let i = 1; i <= totalPages; i++) {
        container.innerHTML += `<button class="page-num ${i === summaryCurrentPage ? 'active' : ''}" onclick="goToSummaryPage(${i})">${i}</button>`;
    }
}

function goToSummaryPage(page) {
    summaryCurrentPage = page;
    renderSummaryModalContent();
}

function changeSummaryPage(dir) {
    summaryCurrentPage += dir;
    const totalPages = Math.ceil(pickingLogs.length / summaryRowsPerPage) || 1;
    if (summaryCurrentPage < 1) summaryCurrentPage = 1;
    if (summaryCurrentPage > totalPages) summaryCurrentPage = totalPages;
    renderSummaryModalContent();
}

// Goal 3: Add in the action the end picking
function quickEndPick(id) {
    const log = pickingLogs.find(l => l.id === id);
    if (!log) return;
    const now = new Date();
    const timeStr = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    log.endTime = timeStr;
    log.duration = calculateDuration(log.startTime, log.endTime);
    db.transaction("pickingLogs", "readwrite").objectStore("pickingLogs").put(log).onsuccess = () => {
        showToast(`Picking ended for ${log.userId}`, "success");
        renderPickingTable();
        if (document.getElementById('summaryModal').style.display === 'block') {
            renderSummaryModalContent();
        }
    };
}

function filterPickTab(tab) {
    currentPickTab = tab;
    pickCurrentPage = 1;
    // Update active tab UI
    const tabs = document.querySelectorAll('.tab-link');
    tabs.forEach(btn => {
        if (btn.innerText === tab) btn.classList.add('active');
        else btn.classList.remove('active');
    });
    renderPickingTable();
}

function renderPickPagination(totalItems) {
    const totalPages = Math.ceil(totalItems / pickRowsPerPage) || 1;
    const container = document.getElementById('pickPaginationNumbers');
    if (!container) return;
    container.innerHTML = '';

    const maxVisible = 3;
    let startPage = Math.max(1, pickCurrentPage - 1);
    let endPage = Math.min(totalPages, startPage + maxVisible - 1);

    if (endPage - startPage < maxVisible - 1) {
        startPage = Math.max(1, endPage - maxVisible + 1);
    }

    if (startPage > 1) {
        container.innerHTML += `<button class="page-num" onclick="goToPickPage(1)">1</button>`;
        if (startPage > 2) container.innerHTML += `<span class="dots">...</span>`;
    }

    for (let i = startPage; i <= endPage; i++) {
        if (i < 1) continue;
        container.innerHTML += `<button class="page-num ${i === pickCurrentPage ? 'active' : ''}" onclick="goToPickPage(${i})">${i}</button>`;
    }

    if (endPage < totalPages) {
        if (endPage < totalPages - 1) container.innerHTML += `<span class="dots">...</span>`;
        container.innerHTML += `<button class="page-num" onclick="goToPickPage(${totalPages})">${totalPages}</button>`;
    }
}

function goToPickPage(page) {
    pickCurrentPage = page;
    renderPickingTable();
}

function changePickPage(dir) {
    pickCurrentPage += dir;
    if (pickCurrentPage < 1) pickCurrentPage = 1;
    renderPickingTable();
}

function deletePick(id) {
    const modal = document.getElementById('confirmModal');
    const confirmBtn = document.getElementById('confirmBtn');
    
    document.getElementById('confirmMessage').innerText = "Are you sure you want to delete this picking log?";
    modal.style.display = 'block';
    
    confirmBtn.onclick = () => {
        const tx = db.transaction("pickingLogs", "readwrite");
        tx.objectStore("pickingLogs").delete(id).onsuccess = () => {
            pickingLogs = pickingLogs.filter(l => l.id !== id);
            renderPickingTable();
            showToast("Log deleted", "danger");
            modal.style.display = 'none';
        };
    };
}

function exportPicking() {
    const search = document.getElementById('pickSearch').value.toLowerCase();
    const zoneSearch = document.getElementById('zoneSearch').value;
    
    // Ensure we export based on the current visible tab filter as well
    const dataToExport = pickingLogs.filter(l => {
        const matchesSearch = (l.userId.toLowerCase().includes(search) || 
                               l.batchId.toLowerCase().includes(search) ||
                               (l.name && l.name.toLowerCase().includes(search)));
        const matchesZone = (zoneSearch === "" || l.zone === zoneSearch);
        const matchesTab = (currentPickTab === 'Picking' ? l.endTime === '-' : l.endTime !== '-');
        
        return matchesSearch && matchesZone && matchesTab;
    });

    if (dataToExport.length === 0) {
        showToast("No logs to export", "warning");
        return;
    }

    let csvContent = "data:text/csv;charset=utf-8,User ID,Name,Batch ID,Zone,Date,Start Time,End Time,Duration\n";
    dataToExport.forEach(log => {
        csvContent += `${log.userId},${log.name},${log.batchId},${log.zone || '-'},${log.date},${log.startTime},${log.endTime},${log.duration}\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    const timestamp = new Date().toISOString().split('T')[0];
    link.setAttribute("download", `sunnyville_picking_export_${timestamp}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}
